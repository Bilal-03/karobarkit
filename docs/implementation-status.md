...
## Business Toolkit expansion — Phase 6 AI assistants

Status: initial public-beta implementation complete on 10 August 2026. Business Name, Pricing Assistant, Startup Cost Estimator and Business Plan Assistant are visible by default under the `phase6-ai-assistants` flag. No paid provider key is required; no-key deployments use deterministic drafts.

The Phase 6 gateway is server-only and provider-neutral. It supports Gemini (`GEMINI_API_KEY`) and Groq (`GROQ_API_KEY`) with `AI_PROVIDER=gemini|groq|auto`; keys are never exposed to the browser. The route requires explicit consent, enforces a streamed 24 KB body cap and eight-per-ten-minute default limit (with an optional bounded `AI_RATE_LIMIT_PER_WINDOW` override and shared atomic rate-limit endpoint), counts only validated requests, applies a configurable provider daily budget and failure circuit breaker, redacts known contact/tax-ID patterns, rejects obvious payment/identity/credential data, uses an immutable system policy and JSON data payload, validates structured provider JSON, rejects provider-created numeric claims while allowing deterministic metric display rounding, filters unsafe claims and falls back to deterministic templates on missing keys, provider errors or schema failures.

Pricing and startup-cost metrics remain local deterministic arithmetic. Pricing solves the target contribution margin after the expected discount, channel fee and shipping assumptions. AI wording is labelled according to provider/fallback mode, can be regenerated and edited, and cannot be copied/downloaded until the user confirms review. Analytics receives tool/status metadata only. Provider retention, quota and usage terms remain external dependencies.

Focused verification currently covers redaction, blocked sensitive-data classes, malformed cost lines, percentage bounds, numeric authority, prompt-data isolation, unsafe-output filtering, rate limits, consent, no-key fallback, provider status and mocked Gemini/Groq structured output. The Phase 6 evaluation manifest is now traceable but intentionally unsigned. Remaining Phase 6 exit gates are provider-backed red-team/evaluation runs, independent fixture signatures, named AI safety review, production secret/quota rehearsal and representative-language human review. See [`docs/product-spec/phase-6-implementation-status.md`](product-spec/phase-6-implementation-status.md).
